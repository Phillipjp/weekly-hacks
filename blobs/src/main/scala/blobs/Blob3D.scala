...
case class Blob3D(coord: Coord3D, size: Int) extends Blob[Coord3D] {

  override def findClosestBlob(blobs: Seq[Blob[Coord3D]]): Blob[Coord3D] = {
  ...
  private [blobs] def breakDistanceTie(blobs: Seq[Blob[Coord3D]]): Blob[Coord3D] = {
  ...
  override def moveTowardsBlob(blob: Blob[Coord3D]): Blob[Coord3D] = {
  ...
  private def as2DBlob(blob: Blob[Coord3D]): Blob2D = Blob2D(Coord2D(blob.coord.x, blob.coord.y), blob.size)
}
