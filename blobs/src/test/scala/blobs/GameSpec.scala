...
class GameSpec extends AnyFlatSpecLike with Matchers{
  ...
  it should "remove the smallest blobs" in{
    ...
    // When
    val actual = new Game(Coord2DWrapper(Seq(Coord2D(0,0))), Blob2DWrapper(blobs)).removeSmallestBlobs(blobs)
  ...
  it should "get the smallest blobs" in {
    ...
    // When
    val actual = new Game(Coord2DWrapper(Seq(Coord2D(0,0))), Blob2DWrapper(blobs)).getSmallestBlobs(blobs)
  ...
  it should "it should move blobs to the next closest blob" in {
    ...
    // When
    val actual = new Game(Coord2DWrapper(Seq(Coord2D(0,0))), Blob2DWrapper(blobs)).moveBlobs(blobs, movableBlobs)
...
//
//    // When
//    val actual = new Game(Coord2DWrapper(Seq(Coord2D(0,0))), Blob2DWrapper(blobs)).mergeBlobs(blobs)
//
//    // Then
//    actual.blobs should contain theSameElementsAs  Seq(Blob2D(Coord2D(4,4),3), Blob2D(Coord2D(1,1),3))
//
//  }
