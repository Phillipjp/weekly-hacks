...
object VigenereCipher extends Cipher[String]{
  ...
  private [vigenere] def makeVigenereDecrypterTable(language: Language): Map[Char, Map[Char, Char]] = {
    language.alphabet.zip((0 until language.alphabet.length).map(shift => CaesarCipher.encrypt(language.alphabet, shift, language).zip(language.alphabet).toMap)).toMap
  }
